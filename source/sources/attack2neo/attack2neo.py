...
def build_label(txt):
    ...
    if txt.startswith('course-of-action'):    return 'Mitigation'
    return 'Unknown'
...
# BUILD ALIASES
# -----------------------------------------------------------------
def build_objects(obj, key, graph):
    ...
    if obj.get('x_mitre_version'):    props['x_mitre_version'] = obj['x_mitre_version']
    if obj.get('external_references'):
        for e in obj['external_references']:
            if e.get("source_name") and e["source_name"] == "mitre-attack":
                props['external_id'] = e["external_id"]
                break
    # if obj.get('external_references'):	props['external_references'] = obj['external_references']
    # create node for the group
    node_main = Node(label, **props)
    # merge node to graph
    graph.merge(node_main, label, 'id')
...
def build_relations(obj, gnames, graph):
    if not gnames.get(obj['source_ref']): return
    if not gnames.get(obj['target_ref']): return

    m = NodeMatcher(graph)

    source = m.match(build_label(obj['source_ref']), id=obj['source_ref']).first()
    target = m.match(build_label(obj['target_ref']), id=obj['target_ref']).first()

    relation = Relationship.type(obj['relationship_type'])

    # Changed the original code here merge by id instead of by name (some nodes have the same names)
    graph.merge(relation(source, target), build_label(obj['source_ref']), 'id')
